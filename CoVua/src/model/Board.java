...
public class Board {
    ...
    // Biến để tránh đệ quy vô hạn
    private boolean checkingCastle = false;
        ...
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                ...
                if (p != null && p.getColor() == opponentColor) {
                    // Tránh gọi getPossibleMoves cho vua đối phương
                    if (p instanceof King) {
                        // Vua chỉ tấn công được các ô xung quanh
                        if (Math.abs(r - kingPos.row) <= 1 && Math.abs(c - kingPos.col) <= 1) {
                            return true;
                        }
                    } else {
                        // Kiểm tra nếu quân có thể đến được vị trí vua
                        if (canPieceAttack(p, kingPos.row, kingPos.col)) {
                            return true;
                        }
                    }
                }
    ...
    // Phương thức mới: Kiểm tra xem quân có thể tấn công ô (row, col) không
    private boolean canPieceAttack(Piece piece, int targetRow, int targetCol) {
        // Kiểm tra đặc biệt cho tốt
        if (piece instanceof Pawn) {
            int direction = (piece.getColor() == PieceColor.WHITE) ? -1 : 1;
            int attackRow = piece.getRow() + direction;
            
            // Tốt chỉ ăn chéo
            if (attackRow == targetRow && 
                (piece.getCol() - 1 == targetCol || piece.getCol() + 1 == targetCol)) {
                return true;
            }
            return false;
        }
        
        // Đối với các quân khác, lấy danh sách nước đi có thể
    ...
    // Kiểm tra có thể nhập thành - SỬA ĐỂ TRÁNH ĐỆ QUY
    public boolean canCastle(PieceColor color, boolean kingside) {
        ...
        try {
            ...
            for (int c = kingCol + step; c != newKingCol + step; c += step) {
                if (c == kingCol) continue; // Bỏ qua vị trí ban đầu
                
                // Kiểm tra xem ô có bị tấn công không
                if (isSquareAttackedByColor(row, c, opponentColor)) {
                    return false;
                }
            }
    ...
    // Phương thức mới: Kiểm tra ô có bị tấn công bởi màu cờ nào đó không
    private boolean isSquareAttackedByColor(int row, int col, PieceColor attackerColor) {
        for (int r = 0; r < 8; r++) {
            for (int c = 0; c < 8; c++) {
                Piece piece = board[r][c];
                if (piece != null && piece.getColor() == attackerColor) {
                    // Đối với vua, chỉ kiểm tra các ô xung quanh
                    ...
                    // Đối với tốt, kiểm tra ăn chéo
                    if (piece instanceof Pawn) {
                        int direction = (piece.getColor() == PieceColor.WHITE) ? -1 : 1;
                        int attackRow = r + direction;
                    ...
                    // Đối với các quân khác, kiểm tra nước đi hợp lệ
                    List<Position> moves = piece.getPossibleMoves(this);
                    for (Position move : moves) {
    ...
    public Board cloneBoard() {
        ...
        copy.checkingCastle = this.checkingCastle;
